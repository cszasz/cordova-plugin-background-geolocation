...
public class LocationUpdateService extends Service implements LocationListener
{
    ...
    public void onCreate() {
        ...
        // Stop-detection PI
        stationaryAlarmPI   = PendingIntent.getBroadcast(this, 0, new Intent(STATIONARY_ALARM_ACTION), PendingIntent.FLAG_MUTABLE);
        registerReceiver(stationaryAlarmReceiver, new IntentFilter(STATIONARY_ALARM_ACTION));

        // Stationary region PI
        stationaryRegionPI  = PendingIntent.getBroadcast(this, 0, new Intent(STATIONARY_REGION_ACTION), PendingIntent.FLAG_CANCEL_CURRENT | PendingIntent.FLAG_MUTABLE);
        registerReceiver(stationaryRegionReceiver, new IntentFilter(STATIONARY_REGION_ACTION));

        // Stationary location monitor PI
        stationaryLocationPollingPI = PendingIntent.getBroadcast(this, 0, new Intent(STATIONARY_LOCATION_MONITOR_ACTION), PendingIntent.FLAG_MUTABLE);
        ...
        singleUpdatePI = PendingIntent.getBroadcast(this, 0, new Intent(SINGLE_LOCATION_UPDATE_ACTION), PendingIntent.FLAG_CANCEL_CURRENT| PendingIntent.FLAG_MUTABLE);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            ...
            PendingIntent pendingIntent = PendingIntent.getActivity(this, 0, main, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_MUTABLE);
    ...
    */
    ...
    public void onLocationChanged(Location location) {
        Log.d(TAG, ""+stopOnTerminate+"- onLocationChanged: " + location.getLatitude() + "," + location.getLongitude() + ", accuracy: " + location.getAccuracy() + ", isMoving: " + isMoving + ", speed: " + location.getSpeed());
        ...
        if (isAcquiringStationaryLocation) {
            ...
            if (++locationAcquisitionAttempts == MAX_SPEED_ACQUISITION_ATTEMPTS) {
                ...
                //Toast.makeText(this, "Return 2 "+locationAcquisitionAttempts, Toast.LENGTH_SHORT).show();
                Log.d(TAG, "Return 2");
                return;
            }
        } else if (isMoving) {
        ...
        if (isDebugging) {
            Toast toast = Toast.makeText(this, "mv:"+isMoving+",loc:"+location.getLatitude() + "x" + location.getLongitude() +",acy:"+location.getAccuracy()+",v:"+location.getSpeed()+",df:"+scaledDistanceFilter, Toast.LENGTH_SHORT);
            toast.show();
            try {
                Thread.sleep(400);
            } catch (Exception ex) {}
    ...
    */
    ...
    private boolean postLocation(com.zencity.cordova.bgloc.data.Location l, LocationDAO dao) {
        ...
        try {
            ...
            Log.i(TAG, "Response received: " + urlConnection.getResponseCode()+":"+urlConnection.getResponseMessage());
            if (responseCode == 200) {
                return true;
            } else {
                return false;
